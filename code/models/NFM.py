...
class NFM(nn.Module):
    def __init__(self, num_features, num_factors, 
        act_function, layers, batch_norm, drop_prob, num_groups=18, pretrain_FM=None):
    ...
    def forward(self, user_features, user_feature_values, item_features, item_feature_values, label, is_testing=False):
        ...
        t_loss = self.criterion(input=FM, target=label)

        if is_testing:
            # for testing, use t_loss to return item embeddings
            #item_feat_emb = self.embeddings(item_features) #[B, IF, d] 
            #item_feature_val =item_feature_values.unsqueeze(dim =-1)
            #t_loss = torch.sum(item_feat_emb*item_feature_val, dim =1)
            t_loss = item_feature_values[:,-self.num_groups:]
 
        return FM.view(-1), t_loss
